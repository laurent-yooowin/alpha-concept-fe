...
export const generatePdfService = {
  ...
  getSuggestedReportFilename(data: {
    reportType?: string;
    client?: string;
    city?: string;
    address?: string;
    ...
    const explicitCity = String(data.city || '').trim();
    ...
    if (!citySource && addressParts.length > 1) {
      ...
      citySource = addressValue.match(/\b\d{5}\s+([^,]+)$/)?.[1]?.trim() || '';
    }
  ...
  /** @deprecated Le PDF canonique est généré uniquement par le backend. */
  ...
  async generateHTMLContent(reportData: any) {
    ...
    if (reportData.photos && reportData.photos.length > 0) {
      ...
      for (const [groupId, groupData] of groupEntries) {
        ...
        const isDirectiveOnlyPhoto = (photo: any) => (
          photo.isDirectiveOnly === true ||
          photo.isDirectiveOnly === 'true' ||
          (!photo.uri && !photo.s3Url && Boolean(photo.userDirectives?.trim()))
        );
        const actualPhotoCount = photos.filter((photo: any) => !isDirectiveOnlyPhoto(photo)).length;
        const isSinglePhoto = actualPhotoCount === 1;
        const isDirectiveOnlyGroup = actualPhotoCount === 0;
        ...
        const photoCountLabel = isDirectiveOnlyGroup
          ? 'Sans photo'
          : `${actualPhotoCount} photo${actualPhotoCount > 1 ? 's' : ''}`;
    ...
    <head>
      ...
      <style>
        ...
        /* Pagination: keep headings with their content without moving whole long groups. */
        .observations-title {
          break-before: page;
        ...
        * {
        ...
        .photo-section {
          /* A long group may flow onto the next page; only its atomic children stay together. */
          break-inside: auto;
    ...
    <body>
      ...
      <div class="conclusion-section">
        <div class="section-header conclusion-title">✅ Conclusion</div>
  ...
  /**
   * Produces the canonical PDF in backend Chromium and stores it on S3.
   * This is shared with the native application.
   */
  async generateServerPDF(reportId: string, fileName: string, forceRegenerate = false) {
    return apiRequest(`/reports/${reportId}/pdf`, {
      method: 'POST',
      body: JSON.stringify({ fileName, forceRegenerate }),
    });
  },


  async generateServerPDFByVisit(visitId: string, fileName: string, forceRegenerate = false) {
    return apiRequest(`/reports/by-visit/${visitId}/pdf`, {
      method: 'POST',
      body: JSON.stringify({ fileName, forceRegenerate }),
    });
  },
  async generateStoreAndSendReportPDF(
    reportId: string,
    payload: {
      fileName: string;
      email: string;
    ...
    return apiRequest(`/reports/${reportId}/send`, {
      method: 'POST',
  ...
  async downloadReportPDF(reportData: any, filename?: string): Promise<boolean> {
    try {
      ...
      if (!reportId) {
        throw new Error('Identifiant du rapport manquant pour la génération PDF serveur');
      }
      ...
      const generated = await this.generateServerPDF(reportId, fileName);
      if (!generated?.url) {
        throw new Error('Le serveur n’a pas généré de PDF valide');
      }

      const response = await filesService.downloadFile(generated.url, 'reports', true);
      const base64 = response?.data?.base64;
      if (!base64) {
        throw new Error('Le PDF généré est vide ou inaccessible');
      }
      ...
      anchor.download = response.data?.fileName || fileName;
