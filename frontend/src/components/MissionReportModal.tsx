...
export default function MissionReportModal({ mission, onClose, initialReportId }: MissionReportModalProps) {
  ...
  const getSuggestedPdfFileName = (report: Report | null = selectedReport) => generatePdfService.getSuggestedReportFilename({
    reportType: 'vc',
    client: report?.client || mission.client,
    address: report?.address || mission.address,
    project: mission.title || report?.title,
  ...
  const handleSendToClient = async () => {
    ...
    setNewRecipientName('');
    setPdfFileName(getSuggestedPdfFileName());
    try {
  ...
  const confirmSendToSelectedRecipients = async () => {
    ...
    const normalizedFileName = normalizePdfFileName(pdfFileName, getSuggestedPdfFileName());
    setPdfFileName(normalizedFileName);
    setShowRecipientsModal(false);
    await sendReportToClient(selectedCc, normalizedFileName);
  };
...
${currentUser ? `Coordonnateur: ${currentUser.firstName} ${currentUser.lastName}` : ''}`;
      ...
      const missionType = mission.type || 'SPS';
      const subject = `Rapport ${missionType} – ${mission.title} – ${selectedReport.createdAt || ''}`;
      ...
      Swal.fire({
        icon: 'error',
        title: 'Erreur',
        text: error instanceof Error ? error.message : 'Échec de l\'envoi du mail.',
      });
    } finally {
  ...
  const handleDownloadReportPdf = async (report: Report, e?: React.MouseEvent) => {
    ...
    try {
      ...
      }, getSuggestedPdfFileName(report));
  ...
  const handleDownloadGeneratedPdf = async () => {
    ...
    try {
      const ok = await generatePdfService.downloadReportPDF({
        ...
        mission: mission.title,
      }, getSuggestedPdfFileName(selectedReport));
