...
export const pdfService = {
  async generateReportPDF(reportData: ReportData, filename?: string): Promise<string | null> {
    try {
      const htmlContent = await this.generateHTMLContent(reportData);

      if (Platform.OS === 'web') {
        return await this.generateWebPDF(htmlContent, filename || reportData.title);
      } else {
        // 
        // console.log('htmlContent >>>', htmlContent)
        return await this.generateNativePDF(htmlContent, filename || reportData.title);
      }
    } catch (error) {
      ...
      await Promise.all(
        Object.entries(photoGroups).map(async ([groupId, group]) => {
          try {
            ...
            const actualPhotoCount = group.photos.filter((photo: any) => !isDirectiveOnlyPhoto(photo)).length;
...
<head>
  ...
  <style>
    @page {
      /* Safe printable area on iOS/Android PDF engines. */
