          ...
          if (visit && visit.photos && visit.photos.length > 0) {
            const loadedPhotos: Photo[] = await Promise.all(visit.photos?.map(async (photo: any) => {
              ...
              return {
                ...
                isAttachedPhoto: photo.isAttachedPhoto === true || String(photo.id || '').startsWith('attach-'),
  ...
  const saveVisit = async (photosParam = [], silent: boolean = false) => {
    ...
    try {
      const visitPhotos = photosData.map((p: any) => ({
        ...
        isAttachedPhoto: p.isAttachedPhoto === true || String(p.id || '').startsWith('attach-'),
                            ...
                            <View style={{ marginBottom: 20, marginTop: 8 }}>
                              ...
                              {group.photos.map((photo, idx) => {
                                const isAttachedOnly =
                                  (photo.isAttachedPhoto === true || String(photo.id || '').startsWith('attach-')) &&
                                  !photo.isDirectiveOnly &&
                                  !photo.isDetailPhoto &&
                                  !photo.isReadabilityPhoto;
