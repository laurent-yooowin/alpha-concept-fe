...
export class MailService {
  ...
  async sendPdfReport(to: string, subject: string, text: string, pdfBuffer: Readable, pdfName: string, cc?: string[]) {
    try {
      ...
      const options: ISendMailOptions = {
        ...
        from: smtpUser ? `"Rapports Automatiques" <${smtpUser}>` : undefined,
      ...
      const info = await this.mailerService.sendMail(options) as {
        accepted?: string[];
        rejected?: string[];
        messageId?: string;
        response?: string;
      ...
      this.logger.log(
        `Rapport remis au serveur SMTP (messageId=${info.messageId || 'inconnu'}, accepted=${info.accepted?.length || 0}, rejected=${info.rejected?.length || 0}, response=${info.response || 'inconnue'})`,
      );
      return {
        success: true,
        message: 'Email remis au serveur d’envoi.',
  ...
  async generatePdfBuffer(htmlContent: string): Promise<Uint8Array<ArrayBufferLike>> {
    ...
    try {
      ...
      // html2pdf captured the report with its screen styles. Keep the same
      // media here so moving the rendering to the backend does not alter the
      // template's spacing, colors or responsive layout.
      ...
      // Chromium can otherwise finish the PDF while a remote logo, an image
      // or a web font is still being decoded. Keep these waits bounded: one
      // unavailable remote asset must not prevent the whole report from being
      // generated.
      await page.evaluate(async () => {
        const waitAtMost = <T>(promise: Promise<T>, timeoutMs: number) =>
          Promise.race([
            promise,
        ...
        const fontsReady = document.fonts
          ...
          : Promise.resolve();
        await waitAtMost(fontsReady, 5_000);
        await waitAtMost(
          Promise.all(
      ...
      // Force exact background/gradient rendering without replacing any of
      // the report template's own CSS rules.
      await page.addStyleTag({
        content: 'html, body { -webkit-print-color-adjust: exact !important; print-color-adjust: exact !important; }',
      });
